...
function readAll() {
    ...
    try {
        ...
        logger.exception("Teamchests konnten nicht geladen werden", error);
        return {};
    }
}
...
function writeAll(data) {
    try {
        ...
        logger.exception("Teamchests konnten nicht gespeichert werden", error);
        return false;
    }
}
...
function loadTeamStorage(teamName) {
    const all = readAll();
    const data = Array.isArray(all[teamName]) ? all[teamName] : [];
    return Array.from({ length: SLOT_COUNT }, (_, i) => data[i] ?? null);
}

function saveTeamStorage(teamName, storage) {
    const all = readAll();
    all[teamName] = storage;
    return writeAll(all);
}
...
function serializeItem(item) {
    ...
    try {
        ...
        if (enchantments?.length) {
            result.enchantments = enchantments.map(e => ({ id: e.type?.id ?? e.typeId ?? e.id, level: e.level }));
        }
    } catch {}
    try {
    ...
    try {
        ...
        if (Array.isArray(data.enchantments)) {
            ...
            for (const enchantment of data.enchantments) {
                try { enchantable?.addEnchantment({ type: enchantment.id, level: Number(enchantment.level) }); } catch {}
            }
        }
        ...
        logger.exception(`Teamchest-Item konnte nicht rekonstruiert werden: ${data.typeId}`, error);
        return undefined;
    }
}

function displayName(item) {
    return item?.nameTag || item?.typeId?.replace(/^minecraft:/, "") || "Leer";
}

function getTeam(player) {
    const team = getPlayerTeam(player);
    return team && getTeams()[team] ? team : null;
}
...
async function deposit(player, teamName, slot) {
    ...
    const form = new ModalFormData().title(`§6Team-Doppelchest – Slot ${slot + 1}`)
        .dropdown("Inventar-Slot", options.map(o => `Slot ${o.slot + 1}: ${displayName(o.item)} x${o.item.amount}`), { defaultValueIndex: 0 });
    try {
        ...
        if (!selected) return;
        const inv = inventory(player);
        const item = inv?.getItem(selected.slot);
        if (!item) {
        ...
        if (storage[slot]) {
            player.sendMessage("§cDieser Teamchest-Slot ist bereits belegt.");
            return;
        }
        storage[slot] = serializeItem(item);
        inv.setItem(selected.slot, undefined);
        if (!saveTeamStorage(teamName, storage)) {
            inv.setItem(selected.slot, item);
            storage[slot] = null;
            return;
        }
        player.sendMessage(`§a${displayName(item)} x${item.amount} wurde in die Team-Doppelchest gelegt.`);
    } catch (error) {
        logger.exception(`Teamchest-Einlagerung fehlgeschlagen: ${player.name}`, error);
    }
}
...
async function slotMenu(player, teamName, slot) {
    ...
    if (stored) {
        form.body(`§f${displayName(stored)} x${stored.amount}\n§7Gemeinsamer Team-Speicher.`)
    ...
    try {
        ...
        if (stored) {
            if (result.selection === 0) {
                const inv = inventory(player);
                if (!inv) return;
                const leftover = inv.addItem(stored);
                if (leftover) {
                    player.sendMessage("§cDein Inventar ist voll. Das Item wurde nicht herausgenommen.");
                    return slotMenu(player, teamName, slot);
                }
                storage[slot] = null;
                saveTeamStorage(teamName, storage);
            } else if (result.selection === 1) {
                await deposit(player, teamName, slot);
        ...
        logger.exception(`Teamchest-Slot-UI fehlgeschlagen: ${player.name}`, error);
    }
}

async function openTeamChest(player) {
    if (!player?.isValid) return;
    const teamName = getTeam(player);
    if (!teamName) {
        player.sendMessage("§cDu bist keinem Team zugeordnet.");
        return;
    }
    const storage = loadTeamStorage(teamName);
    const form = new ActionFormData().title(`§6Team-Doppelchest §7[${teamName}]`)
        .body("§7Gemeinsamer 54-Slot-Speicher für alle Mitglieder deines Teams.");
    for (let i = 0; i < SLOT_COUNT; i++) {
        const item = deserializeItem(storage[i]);
        form.button(item ? `§fSlot ${i + 1}\n§7${displayName(item)} x${item.amount}` : `§8Slot ${i + 1}\n§7Leer`);
    }
    try {
        const result = await form.show(player);
        if (result.canceled || result.selection === undefined) return;
        await slotMenu(player, teamName, result.selection);
    } catch (error) {
        logger.exception(`Teamchest-UI fehlgeschlagen: ${player.name}`, error);
    }
}

export function registerTeamChestCommand(registry) {
    registry.registerCommand({
        name: "siedler:teamchest",
        description: "Öffnet die gemeinsame Team-Doppelchest.",
        permissionLevel: CommandPermissionLevel.Any,
        cheatsRequired: false
    }, (origin) => {
        const player = origin?.sourceEntity?.typeId === "minecraft:player" ? origin.sourceEntity : undefined;
        if (!player) return { status: CustomCommandStatus.Failure };
        system.run(() => openTeamChest(player));
