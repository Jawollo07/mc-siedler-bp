...
system.beforeEvents.startup.subscribe((event) => {
    ...
    registry.registerCommand({ name: "siedler:monster_set", description: "Setzt eine einfache Monster-Config Einstellung.", permissionLevel: OP_PERMISSION, cheatsRequired: false, mandatoryParameters: [{ type: CustomCommandParamType.String, name: "key" }, { type: CustomCommandParamType.String, name: "value" }] }, (origin, args) => {
        ...
        system.run(() => {
            if (!(key in MONSTER_CONFIG) || typeof MONSTER_CONFIG[key] === "object") { player.sendMessage(`§cUnbekannter oder verschachtelter Key: ${key}`); player.sendMessage("§7Nutze /siedler:monster_mob oder /siedler:monster_chance."); return; }
            let value;
    ...
    registry.registerCommand({ name: "siedler:monster_mob", description: "Aktiviert oder deaktiviert ein Monster.", permissionLevel: OP_PERMISSION, cheatsRequired: false, mandatoryParameters: [{ type: CustomCommandParamType.String, name: "type" }, { type: CustomCommandParamType.String, name: "enabled" }] }, (origin, args) => {
        const player = playerOnly(origin); if (!player) return { status: CustomCommandStatus.Failure };
        ...
        system.run(() => { MONSTER_CONFIG.allowedMobs[type] = raw === "true"; player.sendMessage(saveMonsterConfig() ? `§a${type} §7ist jetzt §e${raw === "true" ? "erlaubt" : "blockiert"}§7.` : "§cDie Config konnte nicht gespeichert werden."); });
        return { status: CustomCommandStatus.Success };
    });

    registry.registerCommand({ name: "siedler:monster_chance", description: "Setzt die Spawn-Chance eines Monsters von 0 bis 1.", permissionLevel: OP_PERMISSION, cheatsRequired: false, mandatoryParameters: [{ type: CustomCommandParamType.String, name: "type" }, { type: CustomCommandParamType.Float, name: "chance" }] }, (origin, args) => {
        const player = playerOnly(origin); if (!player) return { status: CustomCommandStatus.Failure };
        ...
        system.run(() => { MONSTER_CONFIG.spawnChances[type] = clamp01(chance); player.sendMessage(saveMonsterConfig() ? `§aSpawn-Chance von ${type} §7auf §e${chance} §7gesetzt.` : "§cDie Monster-Config konnte nicht gespeichert werden."); });
        return { status: CustomCommandStatus.Success };
    });

    registry.registerCommand({ name: "siedler:monster_list", description: "Zeigt die aktuelle Monster-Config.", permissionLevel: OP_PERMISSION, cheatsRequired: false }, (origin) => {
        const player = playerOnly(origin); if (!player) return { status: CustomCommandStatus.Failure };
        system.run(() => {
            ...
            player.sendMessage("§7/siedler:monster_set <key> <value>");
            player.sendMessage("§7/siedler:monster_mob <mob> <true|false>");
            player.sendMessage("§7/siedler:monster_chance <mob> <0-1>");
        });
        return { status: CustomCommandStatus.Success };
    });

    registry.registerCommand({ name: "siedler:monster_reset", description: "Setzt die Monster-Config auf Standard zurück.", permissionLevel: OP_PERMISSION, cheatsRequired: false }, (origin) => {
        const player = playerOnly(origin); if (!player) return { status: CustomCommandStatus.Failure };
        system.run(() => {
